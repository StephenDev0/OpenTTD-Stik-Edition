	...
	NWidget(WWT_PANEL, Colours::Grey),
		NWidget(NWID_HORIZONTAL), SetPadding(WidgetDimensions::unscaled.framerect), SetPIP(0, WidgetDimensions::unscaled.hsep_wide, 0), SetPIPRatio(0, 1, 2),
			NWidget(NWID_VERTICAL), // Vertical column with 'bank balance', 'loan'
				...
				NWidget(WWT_TEXT, Colours::Invalid), SetStringTip(STR_FINANCES_VALUATION_TITLE), SetPadding(WidgetDimensions::unscaled.vsep_normal, 0, 0, 0),
				NWidget(WWT_TEXT, Colours::Invalid), SetStringTip(STR_FINANCES_SHARES_TITLE),
				NWidget(WWT_TEXT, Colours::Invalid), SetStringTip(STR_FINANCES_DIVIDEND_POLICY_TITLE), SetFill(0, 1),
				/* CITYSIM: end. */
			EndContainer(),
			NWidget(NWID_VERTICAL), // Vertical column with bank balance amount, loan amount, and total.
				...
				/* CITYSIM: stock market rows. */
				NWidget(WWT_TEXT, Colours::Invalid, WID_CF_VALUATION_VALUE), SetAlignment(SA_VERT_CENTER | SA_RIGHT | SA_FORCE), SetPadding(WidgetDimensions::unscaled.vsep_normal, 0, 0, 0),
				NWidget(WWT_TEXT, Colours::Invalid, WID_CF_SHARE_STATUS_VALUE), SetAlignment(SA_VERT_CENTER | SA_RIGHT | SA_FORCE),
				NWidget(WWT_DROPDOWN, Colours::Grey, WID_CF_DIVIDEND_POLICY), SetFill(1, 0), SetToolTip(STR_FINANCES_DIVIDEND_POLICY_TOOLTIP),
				/* CITYSIM: end. */
			EndContainer(),
		...
		NWidget(NWID_SELECTION, Colours::Invalid, WID_CF_SEL_BUTTONS),
			NWidget(NWID_HORIZONTAL, NWidContainerFlag::EqualSize),
				...
				NWidget(WWT_PUSHTXTBTN, Colours::Grey, WID_CF_FILE_IPO), SetFill(1, 0), SetStringTip(STR_FINANCES_IPO_BUTTON, STR_FINANCES_IPO_TOOLTIP),
				NWidget(WWT_PUSHTXTBTN, Colours::Grey, WID_CF_PAY_DIVIDEND), SetFill(1, 0), SetToolTip(STR_FINANCES_DIVIDEND_TOOLTIP),
				NWidget(WWT_PUSHTXTBTN, Colours::Grey, WID_CF_VENTURES), SetFill(1, 0), SetStringTip(STR_FINANCES_VENTURES_BUTTON, STR_FINANCES_VENTURES_TOOLTIP),
				/* CITYSIM: end. */
		EndContainer(),
	EndContainer(),
};
...
struct CompanyFinancesWindow : Window {
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_CF_PAY_DIVIDEND:
				Command<Commands::IssueDividend>::Post(STR_ERROR_CAN_T_PAY_DIVIDEND, _ctrl_pressed ? DIVIDEND_INTERVAL * 10 : DIVIDEND_INTERVAL);
				break;
	...
	void OnDropdownSelect(WidgetID widget, int index, int) override
	{
		if (widget == WID_CF_DIVIDEND_POLICY) {
			Command<Commands::SetDividendPolicy>::Post(STR_ERROR_CAN_T_SET_DIVIDEND_POLICY, ClampTo<uint8_t>(index));
			return;
		}
		if (widget == WID_CF_FILE_IPO) {
			Command<Commands::FileIpo>::Post(STR_ERROR_CAN_T_FILE_IPO, ClampTo<uint8_t>(index));
			return;
		}
	}
	/* CITYSIM: end. */
