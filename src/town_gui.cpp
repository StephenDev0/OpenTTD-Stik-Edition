...
public:
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		Rect tr = r.Shrink(WidgetDimensions::scaled.framerect);
			...
			DrawString(tr, GetString(STR_TOWN_VIEW_ZONE_DEMAND_SUMMARY,
					GetTownZoneDemand(this->town, TownZone::Residential),
					GetTownZoneDemand(this->town, TownZone::Commercial),
					GetTownZoneDemand(this->town, TownZone::Industrial)));
			tr.top += GetCharacterHeight(FontSize::Normal);
	...
	{
		uint aimed_height = static_cast<uint>(2 + CargoSpec::town_production_cargoes[TownProductionEffect::Passengers].size() + CargoSpec::town_production_cargoes[TownProductionEffect::Mail].size()) * GetCharacterHeight(FontSize::Normal);
...
static constexpr std::initializer_list<NWidgetPart> _nested_town_game_view_widgets = {
	...
	NWidget(NWID_HORIZONTAL, NWidContainerFlag::EqualSize),
		...
		NWidget(WWT_PUSHTXTBTN, Colours::Brown, WID_TV_ZONES), SetFill(1, 0), SetResize(1, 0), SetStringTip(STR_TOWN_VIEW_ZONES_BUTTON, STR_TOWN_VIEW_ZONES_TOOLTIP), /* CITYSIM */
		NWidget(WWT_RESIZEBOX, Colours::Brown),
	EndContainer(),
};
