...
struct CityBuildToolbarWindow : Window {
	...
	void OnPlaceObject([[maybe_unused]] Point pt, TileIndex tile) override
	{
		...
		Command<Commands::PlacePlayerHouse>::Post(STR_ERROR_CAN_T_BUILD_HOUSE, CcPlaySound_CONSTRUCTION_OTHER, tile, zone);
	}
...
static constexpr std::initializer_list<NWidgetPart> _nested_citybuild_toolbar_widgets = {
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_CAPTION, Colours::DarkGreen), SetStringTip(STR_CITYBUILD_TOOLBAR_CAPTION, STR_TOOLTIP_WINDOW_TITLE_DRAG_THIS),
		NWidget(WWT_STICKYBOX, Colours::DarkGreen),
	EndContainer(),
	NWidget(NWID_HORIZONTAL_LTR),
		NWidget(WWT_IMGBTN, Colours::DarkGreen, WID_CBT_PLACE_HOUSE), SetToolbarMinimalSize(1), SetFill(0, 1), SetSpriteTip(SPR_IMG_TOWN, STR_CITYBUILD_TOOLBAR_PLACE_HOUSE_TOOLTIP),
		NWidget(WWT_IMGBTN, Colours::DarkGreen, WID_CBT_PLACE_COMMERCIAL), SetToolbarMinimalSize(1), SetFill(0, 1), SetSpriteTip(SPR_IMG_COMPANY_GENERAL, STR_CITYBUILD_TOOLBAR_PLACE_COMMERCIAL_TOOLTIP),
		NWidget(WWT_IMGBTN, Colours::DarkGreen, WID_CBT_FUND_INDUSTRY), SetToolbarMinimalSize(1), SetFill(0, 1), SetSpriteTip(SPR_IMG_INDUSTRY, STR_CITYBUILD_TOOLBAR_FUND_INDUSTRY_TOOLTIP),
	EndContainer(),
	NWidget(WWT_PANEL, Colours::DarkGreen, WID_CBT_STATUS), EndContainer(),
};
...
Window *ShowCityBuildToolbar()
{
	...
	CloseWindowByClass(WindowClass::CityBuildToolbar);
